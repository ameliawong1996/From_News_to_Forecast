...
class ModelArguments:
    ...
    lora_weight_for_init: Optional[str] = field(
        default=None,
        metadata={"help": ("The LORA checkpoint for weights initialization. Don't set if you want to train a model from scratch.")},
    )
    ...
    config_overrides: Optional[str] = field(
        default=None,
        metadata={"help": "Override some existing default config settings when a model is trained from scratch. Example: n_embd=10,resid_pdrop=0.2,scale_attn_weights=false,summary_type=cls_index"},
    )
    ...
    target_modules: Optional[str] = field(
        ...
        metadata={"help": "List of module names or regex expression of the module names to replace with Lora. For example, ['q', 'v'] or '.*decoder.*(SelfAttention|EncDecAttention).*(q|v)$' "},
    )
    ...
    prompter_name: str = field(
        default="alpaca",
        metadata={"help": "alpaca for v3 and before, ts_test for v4 and after"}
    )
...
class DataTrainingArguments:
    ...
    validation_saving: Optional[List[str]] = field(
        default=None,
        metadata={"help": "Save the validation results to."},
    )
    ...
    block_size: Optional[int] = field(
        default=None,
        metadata={"help": "Optional input sequence length after tokenization. The training dataset will be truncated in block of this size for training. Default to the model max input length for single sentence inputs (take into account special tokens)."},
    )
...
def train():
    ...
    if os.path.isdir(training_args.output_dir) and training_args.do_train and not training_args.overwrite_output_dir:
        ...
        if last_checkpoint is None and len(os.listdir(training_args.output_dir)) > 0:
            logger.info(f"**********no check point, restart**********")
            raise ValueError(
    ...
    prompter = Prompter(model_args.prompter_name)
    logger.info(f"Using Prompter Version: {model_args.prompter_name}")
    ...
    if training_args.do_eval:
        ...
        metric = evaluate.load("accuracy-ts.py")
        # metric = evaluate.load("accuracy.py")

        def compute_metrics(eval_preds):
            labels, preds = eval_preds
            preds = preds[:, 1:].reshape(-1)
            labels = labels[:, :-1].reshape(-1)
            # return metric.compute(predictions=preds, references=labels)
            return metric.compute(predictions=preds, references=labels, tokenizer=tokenizer, save_path=data_args.validation_saving)
        # layer_norm_names=[]


    logger.info("**********peft model**********")
